...
def roc_threshold(imp_scores, gen_scores):
    ...
    for j, val in enumerate(fpr):
        if val > 0.0001 and not found:
            #print("TDR = {}% @ FDR: {} Threshold = {}".format(tpr[j-1] * 100, fpr[j-1] * 100, thresholds[j-1]))
            return tpr[j-1] * 100


# load the testing features
# old feats = concatenate both features, then normalize
train_feats = np.load('msp_train_feats/msp_train_feats.npy')
train_labels = np.load('msp_train_feats/msp_train_labels.npy')
val_feats = np.load('msp_train_feats/msp_val_feats.npy')
val_labels = np.load('msp_train_feats/msp_val_labels.npy')
...
print("TAR @ FAR 0.0001 = {}".format(start_tar))
...
def main(args):
    ...
    for epoch in range(config.num_epochs):
        ...
        e_dim = config.stages[config.current_stage][1]
        gen_val1_proj = network.extract_features(gen_val1, embedding_dim = e_dim, block_size=1000, verbose=False)
        gen_val2_proj = network.extract_features(gen_val2, embedding_dim = e_dim, block_size=1000, verbose=False)
        ng = gen_val1.shape[0]
        ...
        print("curr: {}; best: {}".format(curr_tar, best_tar))
        summary = tf.Summary()
        summary.value.add(tag='sd4/accuracy', simple_value=curr_tar)
